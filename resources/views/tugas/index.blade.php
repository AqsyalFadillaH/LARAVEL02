        ...
        <div class="flex-1 p-8">
            ...
            <div class="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p class="text-gray-500">Kelola tugas kuliah mahasiswa</p>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 card-hover">
                <h3 class="text-xl font-semibold text-gray-800 mb-4">Data Tugas</h3>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-blue-50 text-blue-700">
                                <th class="px-4 py-3 font-medium rounded-l-lg">Judul Tugas</th>
                                ...
                                <th class="px-4 py-3 font-medium rounded-r-lg">File</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($tugas as $t)
                                <tr class="border-b border-gray-100 hover:bg-blue-50 transition">
                                    <td class="px-4 py-3">{{ $t->judul_tugas }}</td>
                                    <td class="px-4 py-3">{{ $t->mata_kuliah }}</td>
                                    <td class="px-4 py-3">{{ $t->deskripsi }}</td>
                                    <td class="px-4 py-3">{{ $t->deadline }}</td>
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-1 rounded-full text-sm 
                                            {{ $t->file_path ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                                            {{ $t->file_path ? 'Ada' : 'Tidak ada' }}
                                        </span>
                                    </td>
                                </tr>
                            @endforeach
